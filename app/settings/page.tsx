  ...
  useEffect(() => {
    (async () => {
      ...
      setSettings((prev) => ({
        notifications: {
          push:
            typeof s?.notifications?.push === 'boolean'
              ? s.notifications.push
              : prev.notifications.push,
          email:
            typeof s?.notifications?.email === 'boolean'
              ? s.notifications.email
              : prev.notifications.email,
          sms:
            typeof s?.notifications?.sms === 'boolean'
              ? s.notifications.sms
              : prev.notifications.sms,
        },
        privacy: {
          profileVisible:
            typeof s?.privacy?.profileVisible === 'boolean'
              ? s.privacy.profileVisible
              : prev.privacy.profileVisible,
        },
        preferences: {
          language: s?.preferences?.language || prev.preferences.language,
          darkMode:
            typeof s?.preferences?.darkMode === 'boolean'
              ? s.preferences.darkMode
              : prev.preferences.darkMode,
    ...
    try {
      const payload = {
        ...
        preferences: {
          ...
          ...(category === 'preferences' ? { [key]: value } : {}),
        },
      };
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={saveProfile}
                disabled={saving}
                className="px-5 py-2 rounded-lg text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-70"
              >
                  ...
                  <div key={itemIndex} className="p-4">
                    {item.type === 'toggle' ? (
                      ...
                      >
                        ...
                        {item.showArrow && <ChevronRight className="w-5 h-5 text-gray-400" />}
                      </Link>
                    )}
                  </div>
                ))}
