...
export async function GET(req: NextRequest) {
  try {
    ...
    const byUser = new Map<string, { user_id: string, user_email: string | null, first_open_at: string }>()
    for (const r of firstOpens || []) {
      if (!r.user_id) continue
      if (!byUser.has(r.user_id)) {
        byUser.set(r.user_id, { user_id: r.user_id, user_email: r.user_email || null, first_open_at: r.created_at })
      }
    }
    ...
    const installed: Array<{ id: string, name: string | null, phone: string | null, avatar_url: string | null, email: string | null, first_open_at: string }>=[]
    const not_installed: Array<{ id: string, name: string | null, phone: string | null, avatar_url: string | null, created_at: string }>=[]
    ...
    for (const p of profiles || []) {
      ...
      if (m) {
        installed.push({ id: p.id, name: p.name || null, phone: p.phone || null, avatar_url: p.avatar_url || null, email: m.user_email || null, first_open_at: m.first_open_at })
    ...
    for (const p of profiles || []) {
      if (!installedIds.has(p.id)) {
        not_installed.push({ id: p.id, name: p.name || null, phone: p.phone || null, avatar_url: p.avatar_url || null, created_at: p.created_at })
      }
    }

    return Response.json({ from, to, installed_count: installed.length, not_installed_count: not_installed.length, installed, not_installed }, { headers: corsHeaders(req) })
  } catch (e: any) {
