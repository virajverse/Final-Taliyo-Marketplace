...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!device_id || !user_id) {
      return Response.json({ error: 'device_id and user_id required' }, { status: 400, headers: corsHeaders(req) })
    }

    const updates = { user_id, user_email: user_email || null }

    const [a, b] = await Promise.all([
      supabaseAdmin.from('pwa_installs').update(updates).eq('device_id', device_id).is('user_id', null),
      supabaseAdmin.from('pwa_install_events').update(updates).eq('device_id', device_id).is('user_id', null),
    ])

    if (a.error) throw a.error
    if (b.error) throw b.error

    return Response.json({ ok: true, updated_installs: a.count || null, updated_events: b.count || null }, { headers: corsHeaders(req) })
  } catch (e: any) {
