...
const BookingModal: React.FC<BookingModalProps> = ({ isOpen, onClose, service }) => {
  ...
  const validate = () => {
    ...
    else if (form.whatsappNumber.replace(/\D/g, '').length < 10)
      v.whatsappNumber = 'Enter a valid WhatsApp number';
    if (form.requirements && form.requirements.trim() && form.requirements.trim().length < 10)
      v.requirements = 'Please add a bit more detail (min 10 chars)';
    setValidation(v);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isLoggedIn) {
      ...
      try {
        toast.info('Please sign in and verify your email.');
      } catch {}
      return;
    }
    if (!validate()) return;
  ...
  const cartSubtotal = cartPreview.reduce(
    (sum, it) => sum + (Number((it as any)?.price_min) || 0) * ((it as any)?.quantity || 1),
    0,
  );
    ...
    <div className="fixed inset-0 bg-black/50 z-[70] flex items-start md:items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl md:max-w-3xl">
        ...
        >
          <div className="px-6 py-5 pb-28 space-y-6 flex-1">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {validation.fullName && (
                    <p className="text-xs text-red-600 mt-1">{validation.fullName}</p>
                  )}
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <div>
                  ...
                  />
                  {validation.email && (
                    <p className="text-xs text-red-600 mt-1">{validation.email}</p>
                  )}
                </div>
                <div>
                  ...
                  />
                  {validation.whatsappNumber && (
                    <p className="text-xs text-red-600 mt-1">{validation.whatsappNumber}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              {validation.requirements && (
                <p className="text-xs text-red-600 mt-1">{validation.requirements}</p>
              )}
            </div>
            ...
            <div>
              ...
              >
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Supports: JPG, PNG, SVG, PDF, TXT (Max 10MB each, 5 files total)
                </p>
              </div>
              {files.length > 0 && (
          ...
          <div
            className="sticky bottom-0 z-10 px-6 py-4 border-t border-gray-200 bg-white shadow-[0_-1px_8px_rgba(0,0,0,0.08)]"
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 w-full sm:w-auto"
              >
                Cancel
              </button>
              <button
                ...
                className="px-5 py-2 rounded-lg text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-70 w-full sm:w-auto"
              >
